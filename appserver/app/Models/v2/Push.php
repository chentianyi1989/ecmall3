...
class Push extends BaseModel {
    ...
    public static function getSystemList(array $attributes)
    {
        ...
        $model = Push::where('status', 1)->where('message_type', 1)
    ...
    public static function getOrderList(array $attributes)
    {
        ...
        $model = Push::where('status', 1)->where('message_type', 2)->where('user_id', $uid)->orderBy('created_at','DESC');
    ...
    public static function unread(array $attributes)
    {
        ...
        if(intval($after) == 0)
        {
            $after = Member::where('user_id', $uid)->value('reg_time');
        }
        ...
        if(isset($type)){
            switch ($type) {
                //如果是给系统消息
                case 1:
                    $count = Push::where('status', 1)->where('message_type', 1)
                             ->where('created_at', '>', date('Y-m-d H:i:s', $after))->count();
                    break;
                //如果是给订单消息　　
                case 2:
                    if($uid = Token::authorization()){
                         $count = Push::where('status', 1)->where('message_type', 2)->where('user_id', $uid)
                                ->where('created_at', '>', date('Y-m-d H:i:s', $after))->count();
                    }else{
                         $count = 0;
                    }
            ...
            $count = Push::where('status', 2)
                        ->where('created_at', '>', date('Y-m-d H:i:s', $after))->where('message_type', 1)->count();

            if($uid = Token::authorization()){
                $user_count = Push::where('status', 1)->where('message_type', 2)->where('user_id', $uid)
                        ->where('created_at', '>', date('Y-m-d H:i:s', $after))->count();
