(function () {
    ...
    function SigninController($scope, $http, $window, $location, $state, $rootScope, API, ENUM, ConfigModel) {
      ...
      $scope.isMobileSigninEnable = ConfigModel.getFeature()['signin.mobile'];
      $scope.isDefaultSigninEnable = ConfigModel.getFeature()['signin.default'];

      $scope.userNameHine = "请输入用户名/邮箱/手机号";

      if($scope.isMobileSigninEnable &&$scope.isDefaultSigninEnable ){
        $scope.userNameHine = "请输入用户名/邮箱/手机号";
      } else {
        if($scope.isMobileSigninEnable){
          $scope.userNameHine = "请输入手机号";
        } else {
          $scope.userNameHine = "请输入用户名/邮箱";
        }
      }
