(function () {
	...
	ConfirmController.$inject = ['$scope', '$state', '$rootScope', '$stateParams', 'API', 'ConfirmProductService', 'ExpressSelectService', 'InvoiceSelectService', 'PaymentModel', 'ConfigModel'];

	function ConfirmController($scope, $state, $rootScope, $stateParams, API, ConfirmProductService, ExpressSelectService, InvoiceSelectService, PaymentModel,ConfigModel) {
    ...
    $scope.isScoreEnable = ConfigModel.getFeature()['score']
		...
		if(ConfirmProductService.product){
				...
				for (var j = 0; j < propertiesLength; j++) {
					...
					for (var k = 0; k < attrsLength; k++) {
						...
						if (parseInt(attrItem.id) == attrs[i]) {
							...
							if ( parseFloat(attrItem.attr_price) ) {
			...
			$scope.totalPrice = product_price * ConfirmProductService.amount;
		...
		function _touchExpress() {
			...
			ExpressSelectService.shopId = $scope.selectedGoods[0].product.shop;
