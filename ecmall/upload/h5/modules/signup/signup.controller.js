(function () {
    ...
    SignupController.$inject = ['$scope', '$http', '$window', '$location', '$state', '$stateParams', 'API','ENUM','AppAuthenticationService', '$interval','ConfigModel'];

    function SignupController($scope, $http, $window, $location, $state, $stateParams, API,ENUM, AppAuthenticationService, $interval ,ConfigModel) {
		...
		$scope.isMobileSigninEnable = ConfigModel.getFeature()["signup.mobile"];
    $scope.isDefaultSigninEnable = ConfigModel.getFeature()["signup.default"];

		$scope.touchTabMobile = _touchTabMobile;
		$scope.touchTabEmail = _touchTabEmail;

      if(!$scope.isMobileSigninEnable && $scope.isDefaultSigninEnable ){
