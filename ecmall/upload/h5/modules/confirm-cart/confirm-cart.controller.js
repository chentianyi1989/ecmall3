(function () {
  ...
  ConfirmCartController.$inject = ['$scope', '$http', '$location', '$state', '$rootScope', '$timeout', '$stateParams', 'API', 'ENUM', 'ConfirmCartService', 'ExpressSelectService', 'InvoiceSelectService', 'PaymentModel', 'ConfigModel'];

  function ConfirmCartController($scope, $http, $location, $state, $rootScope, $timeout, $stateParams, API, ENUM, ConfirmCartService, ExpressSelectService, InvoiceSelectService, PaymentModel, ConfigModel) {
    ...
    $scope.isInvoiceEnable = ConfigModel.getFeature()['invoice'];
    ...
    $scope.touchCoupon = _touchCoupon;
    ...
    $scope.shop = ConfirmCartService.shop;

    $scope.couponCount = '0';

    if ($scope.coupon && $scope.coupon.name) {
      $scope.couponCount = $scope.coupon.name;
    }
    ...
    function _touchCoupon() {
      var goods = $scope.goods;

      var totalPrice = 0;

      for (var key in goods) {
        totalPrice += goods[key].price * goods[key].amount;
      }


      $state.go('coupon-select', {
    ...
    function _touchExpress() {
      ...
      ExpressSelectService.shopId = goods[0].product.shop;
    ...
    function _refreshCoupon() {
      ...
      API.coupon.available(params).then(function (data) {
        ...
        $scope.couponCount = data.paged.total;
    ...
    function _reloadPrice() {
      ...
      if ($scope.isB2B2C) {
        for (var i = 0; i < goods.length; ++i) {
          products.push({goods_id: goods[i].product.id, property: goods[i].attrs, num: goods[i].amount});
        }
      } else {
        for (var i = 0; i < goods.length; ++i) {
          products.push({goods_id: goods[i].product.id, property: goods[i].attrs.split(','), num: goods[i].amount});
        }
      }
      ...
      params.shop = goods[0].product.shop;
