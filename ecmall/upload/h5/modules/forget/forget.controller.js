(function () {
	...
	ForgetController.$inject = ['$scope', '$http', '$window', '$location', '$state', '$stateParams', 'API', 'ENUM', 'AppAuthenticationService', '$interval', 'ConfigModel'];

	function ForgetController($scope, $http, $window, $location, $state, $stateParams, API, ENUM, AppAuthenticationService, $interval,ConfigModel) {

	  $scope.isDefaultFindpassEnable = ConfigModel.getFeature()['findpass.default'];
	  $scope.isMobileFindpassEnable = ConfigModel.getFeature()['findpass.mobile'];
		...
		if($scope.isDefaultFindpassEnable && !$scope.isMobileFindpassEnable){
