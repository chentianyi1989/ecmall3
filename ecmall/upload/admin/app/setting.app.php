...
//            $data['page_size']              = $_POST['page_size'];
    ...
    function email_setting()
    {
        ...
        {
            $arr['MAIL_DRIVER']       = ($_POST['email_type'] == 1) ? 'smtp' : 'email';
            $data['email_type']       = $_POST['email_type'];
            $arr['MAIL_HOST']         = $data['email_host']     = $_POST['email_host'];
            $arr['MAIL_PORT']         = $data['email_port']     = $_POST['email_port'];
            $arr['MAIL_FROM_ADDRESS'] = $data['email_addr']     = $_POST['email_addr'];
            $arr['MAIL_USERNAME']     = $data['email_id']       = $_POST['email_id'];
            $arr['MAIL_PASSWORD']     = $data['email_pass']     = $_POST['email_pass'];
            $data['email_test']       = $_POST['email_test'];

            $this->create_app_env($arr, 'appserver');
            $model_setting->setAll($data);
...
//            $data['auto_evaluate']      = $_POST['auto_evaluate'];
     ...
     *
     */
    function sms_sign_setting()
    {
        $yunqioauth_model = &m("yunqioauth");
        $yunqi_member = $yunqioauth_model->get_yunqi_member();
        if (!$yunqi_member) {
            $this->show_message('yunqi_account_inactive_notice',
                'back_list',    'index.php?app=setting&act=base_setting'
            );
            return;
        }
        $model_setting = &af('settings');
        ...
        if (!IS_POST){
            ...
            $this->assign('setting', $setting);
            $this->display('setting.sms_sign_setting.html');
        }else{
            $data['sms_sign'] = trim($_POST['sms_sign']);
            $data['sms_sign'] = str_replace(array('【','】'), '', $data['sms_sign']);
            $res = $yunqioauth_model->smsContentSave($data['sms_sign'], $msg);
            if ($res) {
                $model_setting->setAll($data);
                $this->show_message('edit_sms_sgin_setting_successed');
            }else{
