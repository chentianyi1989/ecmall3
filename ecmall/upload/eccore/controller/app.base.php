...
class BaseApp extends Object
{
     ...
     *
     */
    function create_app_env($arr,$file='appserver')
    {
        $replace = array();
        $url = $arr['SHOP_URL'];
        if($file == 'appserver'){
            $path_tmp = ROOT_PATH.'/../appserver/';
            $path = $path_tmp.'.env';
            ...
            $arr['TERMS_URL'] = $url.'/index.php?app=article&act=system&code=eula';
            $arr['ABOUT_URL'] = $url.'/index.php?app=article&act=system&code=eula';
            foreach ($arr as $k => $v) {
                $replace['#'.$k.'.*#i'] = $k.'='.$v;
            }
        ...
        if($file == 'h5'){
            ...
            $api_url ='http://api.'.str_replace('www.', '', parse_url($url, PHP_URL_HOST));
