...
DROP TABLE IF EXISTS `ecm_dly_corp`;
CREATE TABLE `ecm_dly_corp` (
...
CREATE TABLE `ecm_member` (
  ...
  `passport_type` varchar(10) DEFAULT NULL COMMENT '类型 yunqi 云起免登超管',
  `passport_yunqi_code` varchar(50) DEFAULT NULL COMMENT '云起短信物流token',
  `yunqiexp_active` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否已激活云起物流',
  PRIMARY KEY  (`user_id`),
  KEY `user_name` (`user_name`),
  KEY `email` (`email`),
  KEY `outer_id` (`outer_id`),
  KEY `passport_type` (`passport_type`)
) TYPE=MyISAM;
...
CREATE TABLE `ecm_order` (
  ...
  `out_trade_sn` varchar(50) NOT NULL default '',
...
DROP TABLE IF EXISTS `ecm_proceeds_log`;
CREATE TABLE `ecm_proceeds_log` (
  ...
  `payment_id` int(10) unsigned DEFAULT NULL COMMENT '支付方式id',
  `payment_name` varchar(100) DEFAULT NULL,
  `payment_code` varchar(20) NOT NULL DEFAULT '',
  `out_trade_sn` varchar(50) NOT NULL DEFAULT '',
  `pay_time` int(10) unsigned DEFAULT NULL,
  ...
  `type` enum('income','expenses','modify') NOT NULL COMMENT '支付类型：income:收入 expenses:支出  modify:编辑修改',
  `op_id` int(11) unsigned NOT NULL DEFAULT '0' COMMENT '管理员id',
  `op_name` varchar(20) DEFAULT NULL COMMENT '操作员姓名',
  PRIMARY KEY (`log_id`),
  KEY `order_sn` (`order_sn`,`seller_id`),
  KEY `seller_name` (`seller_name`),
  KEY `buyer_name` (`buyer_name`),
...
CREATE TABLE `ecm_shipping` (
  ...
  `cod_regions` text,
  `corp_id` int(10) NOT NULL DEFAULT '0' COMMENT '物流公司id',
  `enabled` tinyint(3) unsigned NOT NULL default '1',
  `sort_order` tinyint(3) unsigned NOT NULL default '255',
...
CREATE TABLE IF NOT EXISTS `ecm_banner` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `ecm_device` (
...
CREATE TABLE IF NOT EXISTS `ecm_push` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  ...
  `status` tinyint(4) NOT NULL DEFAULT '0' COMMENT '推送状态 0 未发送 1 已发送',
  `created_at` timestamp NULL DEFAULT NULL,
  `updated_at` timestamp NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
...
--
-- 表的结构 `ecm_search_history`
--

CREATE TABLE IF NOT EXISTS `ecm_search_history` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `ecm_sns` (
  `user_id` int(11) NOT NULL,
