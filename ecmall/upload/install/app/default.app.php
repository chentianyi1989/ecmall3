...
class DefaultApp extends InstallerApp
{
    ...
    function install_done()
    {
        ...
        setcookie('__INTECODE__', $_code, 0, '/', NULL, NULL, TRUE);
        ...
        $config = array(
            ...
            'SHOP_DB_PREFIX' => $db_prefix,
            'SHOP_URL' => $site_url,
            'SHOP_H5' => $site_url.'/h5',
            'TOKEN_SECRET' => md5(time() . mt_rand(0,1000))
        );
        /* appserver创建.env文件 */
        $create_app = $this->create_app_env($config, 'appserver');
        /* 创建h5配置文件 */
        $create_h5 = $this->create_app_env($config, 'h5');

        if (!$create_app || !$create_h5) {
            show_process(r(Lang::get('setup_app_config'), false), 'parent.show_warning("' . Lang::get('setup_app_config_error') . '");');
            return false;
        }
        show_process(r(Lang::get('setup_app_config'), true));
        /* 锁定安装程序 */
        touch(LOCK_FILE);
