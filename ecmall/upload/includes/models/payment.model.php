...
class PaymentModel extends BaseModel
{
    ...
    function get_enabled($store_id)
    {
        ...
        $wap_pay = array('wechat', 'alipaywap');
        $pc_pay = array_merge(array_diff($white_list, $wap_pay));  // 去除移动端的两个支付方法(微信支付、手机支付宝支付)
        $payment_code = db_create_in($pc_pay);
        return $this->find(array(
            'conditions'    => "store_id={$store_id} AND enabled=1 AND payment_code" . $payment_code,
            'order'         => 'sort_order',
        ));
    }
