 ...
 *    @return    后台某app连接接口  
 */
function &ls($app_name)
{
    static $ls = null;
    if ($ls === null)
    {
        include(ROOT_PATH . '/admin/app/backend.base.php');
        include(ROOT_PATH . '/admin/app/'.$app_name.'.app.php');
        $class_name  = ucfirst($app_name) . 'App';
        $ls = new $class_name();
    }
 ...
 *
 *    @author    Pangxp
 *    @param     int $proceeds_status
 *    @return    string
 */
function proceeds_status($proceeds_status)
{
    $lang_key = '';
    switch ($proceeds_status)
    {
        case PROCEEDS_SUCC:
            $lang_key = 'proceeds_succ';
        break;
        case PROCEEDS_FAIL:
            $lang_key = 'proceeds_fail';
        break;
    }
 ...
 *  @return  string
 */
function platform_translator($platform){
...
function iframe_source_encode($product)
{
    ...
    $yunqi_member = $member_model->get(array(
        "conditions" => "passport_type = 'yunqi'",
        "fields" => "user_id, passport_uid, passport_type, passport_yunqi_code, yunqiexp_active",
    ));
    $source[] = base64_encode($product);
    $source[] = $yunqi_member["passport_uid"];
    $source[] = $yunqi_member["passport_yunqi_code"];
    $source[] = time();
    $source[] = dirname(site_url());
