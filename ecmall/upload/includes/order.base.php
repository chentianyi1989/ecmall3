...
class BaseOrder extends Object
{
     ...
     *    @return    array
     */
    function _get_my_coupon($store_id, $user_id)
    {
        if (!$store_id || !$user_id){
            return array();
        }
        ...
        $sql = "SELECT * 
FROM {$model_coupon->_prefix}coupon c
LEFT JOIN {$model_coupon->_prefix}coupon_sn cs ON c.coupon_id = cs.coupon_id
LEFT JOIN {$model_coupon->_prefix}user_coupon uc ON cs.coupon_sn = uc.coupon_sn
WHERE uc.user_id ={$user_id}
AND c.store_id ={$store_id}
AND remain_times >0";
        $coupon_data = $model_coupon->getAll($sql);
        $time = gmtime();
        $coupon_list = array();
        foreach ($coupon_data as $key => $value) {
            if (($value['start_time'] <= $time) && ($value['end_time'] == 0 || $value['end_time'] > $time)) {
