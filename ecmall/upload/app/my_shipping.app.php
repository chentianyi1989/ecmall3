...
/*            array(
     ...
     *   物流公司
     */
    function _get_dly_corp()
    {
        $model_dlycorp = &m('dlycorp');
        $dlycorps = $model_dlycorp->find(array(
                'fields' => 'corp_id, name',
                'order' => 'corp_id',
            ));
        $tmp  = array();
        if ($dlycorps)
        {
            foreach ($dlycorps as $key => $value)
            {
                $tmp[$value['corp_id']] = $value['name'];
            }
        }
        $this->assign('dlycorps', $tmp);
    }
}
