...
class Find_passwordApp extends MallbaseApp
{
     ...
     *
     */
    function sms_find_password()
    {
       if(!IS_POST)
       {
           $this->import_resource('jquery.plugins/jquery.validate.js');
           $this->display("sms_find_password.html");
       }
       else
       {
           $addr = $_SERVER['HTTP_REFERER'];
           if (empty($_POST['username']) || empty($_POST['sms_vcode']) || empty($_POST['captcha']))
           {
               $this->show_warning("unsettled_required",
                   'go_back', $addr);
               return ;
           ...
           if ($_POST['sms_vcode'] != $_SESSION[$_POST['username'].'-sms_code'])
           {
