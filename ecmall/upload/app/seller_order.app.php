...
class Seller_orderApp extends StoreadminbaseApp
{
    ...
    function view()
    {
        ...
        $logi_info = $yunqioauth_model->get_logistics_trace($order_id);
        if (isset($logi_info['status']) && $logi_info['status'] == 'succ') {
            $this->assign('logi', $logi_info['data']);
        }
    ...
    function shipped()
    {
        ...
        else
        {
            ...
            /* 物流消息推送app */
            $model_push = &m('leancloud');
            $model_push->delivery_msg_push($order_id, $is_edit);
