...
class MemberApp extends MemberbaseApp
{
    ...
    function register()
    {
        ...
        else
        {
            ...
            /* 手机号作为用户名注册的检测短信验证码是否正确 */
            if (is_moblie($_POST['user_name']) && ($_POST['sms_vcode'] != $_SESSION[$_POST['user_name'].'-sms_code'])) {
                $this->show_warning('sms_vcode_error');
                return;
            }
     ...
     *    @return    json
     */
    function ajax_validate_sms()
    {
        $time= time();
        $date = date('Ymd',$time);
        $pre_date = date('Ymd',$time-64000);
        $ip = ($_SERVER["HTTP_VIA"]) ? $_SERVER["HTTP_X_FORWARDED_FOR"] : $_SERVER["REMOTE_ADDR"];
        $file_path = ROOT_PATH."/temp/logs/";
        if(file_exists($file_path.$pre_date.".txt")) unlink($file_path.$pre_date.".txt");//删除昨日记录
        if(file_exists($file_path.$date.".txt")){
            $send_limit_ip = file_get_contents($file_path.$date.".txt");
            $data = unserialize($send_limit_ip);
            if(isset($data[$ip]) && $data[$ip]>9){
                echo ecm_json_encode(array('done' => 'fail' , 'msg' => Lang::get("ip_limit")));exit;
            }
        }
        $captcha = empty($_POST['captcha1']) ? '' : strtolower(trim($_POST['captcha1']));
        if(!$captcha || base64_decode($_SESSION['captcha']) != $captcha){
            echo ecm_json_encode(array('done' => 'fail' , 'msg' => Lang::get("captcha_error")));exit;
        }
        $mobile = $_POST['phone'] ? $_POST['phone'] : false;
        $is_send = 'fail';
        if($mobile){
            $sms_code = mt_rand(100000,999999);
            $smsModel = &m("sms");
            $_SESSION[$mobile.'-sms_code'] = $sms_code;
            $is_send = $smsModel->send($mobile, sprintf(Lang::get("captcha_content"), $sms_code), '', $msg);
            $is_send = $is_send ? 'succ' : 'fail';
        }else{
            $msg = Lang::get("phone_empty");
        }
        //短信发送限制
        if($is_send == 'succ'){
            ...
            file_put_contents($file_path.$date.".txt",serialize($data));
        }
        echo ecm_json_encode(array('done' => $is_send, 'msg' => $msg));exit;
    }
     ...
     *  @return void
     */
    function ajax_validate_sms_code()
    {
        $sms_code = empty($_GET['sms_vcode']) ? '' : trim($_GET['sms_vcode']);
        $phone = empty($_GET['phone']) ? '' : trim($_GET['phone']);
        if ($sms_code && $sms_code == $_SESSION[$phone.'-sms_code']) {
            echo ecm_json_encode(true);exit();
        } else {
            echo ecm_json_encode(false);exit();
        }
    }
}
