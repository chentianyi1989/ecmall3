...
class FrontendApp extends ECBaseApp
{
    ...
    function FrontendApp()
    {
        ...
        {
            // 关店不影响授权证书平台验证session
            if (isset($_GET['act']) && $_GET['act'] == 'license' && isset($_POST['session_id'])) {
    ...
    function display($tpl)
    {
        ...
        // $this->assign('h5_site_url', SITE_URL.'/h5');
...
class MemberbaseApp extends MallbaseApp
{
    ...
    {
        /* 只有登录的用户才可访问 */
        if (!$this->visitor->has_login && !in_array(ACT, array('login', 'register', 'check_user', 'ajax_validate_sms', 'ajax_validate_sms_code')))
        {
            if (!IS_AJAX)
            {
                header('Location:index.php?app=member&act=login&ret_url=' . rawurlencode($_SERVER['PHP_SELF'] . '?' . $_SERVER['QUERY_STRING']));
